    ...
    <div class="card-content" style="padding-bottom: 0px">
      ...
      <div>
        ...
        <span
          ...
          v-text="'TRAPI ' + api?.info?.['x-trapi']?.version"
        >
        </span>
        <span v-if="api?.info?.['x-translator']?.component" class="versionBadge indigo darken-2"
          >Translator: {{ api?.info?.['x-translator']?.component }}</span
        >
        <span v-if="bt_tag && bt_tag.name == 'biothings'" class="versionBadge grey darken-2">
          BioThings API
        </span>
        <router-link
          v-if="user && api._meta.username === user.login"
          :data-tippy-status="api?._status?.refresh_status || 'n/a'"
          to="/dashboard"
          class="versionBadge light-blue pointer tipped"
        >
          My&nbsp;API
          <template
            v-if="
              api?._status?.refresh_status &&
              ![200, 299, '200', '299'].includes(api?._status?.refresh_status)
            "
          >
            <span
              class="red white-text"
              style="padding: 2px 4px; border-radius: 10px"
              :id="'myAPIErr' + api._id"
            >
              has issues
            </span>
      ...
      <div class="full-width grey-text">
        <template v-for="(tag, index) in api.tags" :key="tag.name">
          <router-link
            :to="$route.path + '?tags=' + tag.name"
            @click="googleAnalytics('Registry_Tag', tag.name)"
            class="blue-text"
          >
