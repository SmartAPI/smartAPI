<template>
  <main id="ui-app" class="indexBackground uiBack" style="width: 100%;">
    ...
    <div v-if="data.ready" class="white p-1" style="position: absolute; top: 80px; left: 20px;">
        <span v-if="data?.api?.openapi " class="versionBadge green">
          OAS3
        </span>
        <span v-else-if="data?.api?.swagger " class="versionBadge blue">
          Swagger2
        </span>
        <span v-if="data?.api?.info?.['x-trapi']?.version" class="versionBadge pink lighten-2" v-text="'TRAPI ' + data?.api?.info?.['x-trapi']?.version">
        </span>
        <span v-if="data?.api?.info?.['x-translator']?.component" class="versionBadge indigo darken-2">Translator: {{data?.api?.info?.['x-translator']?.component}}</span>
        <span v-if="data?.api?.tags && data?.api?.tags.some((item) => item?.name == 'biothings')" class="versionBadge grey darken-2">
          BioThings API
        </span>
      </div>
