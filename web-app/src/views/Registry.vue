    ...
    <div id="app" style="height: 100%" class="white container-fluid" v-cloak>
      ...
      <div class="row">
        <div class="col l2 s12 hide-on-med-and-down hide-on-small-only sidebar">
          ...
          <ul class="collapsible">
            <li>
              <div class="collapsible-header grey lighten-3 grey-text">
                <span
                  >Tags <b class="light-blue-text">({{ tags.length }})</b></span
                >
              </div>
                ...
                <div class="collection">
                  ...
                  <template v-for="tag in tagSearchResults" :key="tag">
                    <a
                      ...
                      style="padding: 2px"
                      :class="{
                        disable: tag.name.toLowerCase() === specialTagOriginalName.toLowerCase(),
                        active: tag.active,
                        blue:
                          tag.active &&
                          tag.name.toLowerCase() !== specialTagOriginalName.toLowerCase(),
                        green:
                          tag.active &&
                          tag.name.toLowerCase() === specialTagOriginalName.toLowerCase(),
                        'white-text': tag.active
                      }"
                    ...
                    >
                      <small
                        >{{ tag.name }} <span class="bold">({{ tag.count }})</span></small
                      >
                    </a>
                  </template>
                  <!-- no search - ALL -->
            ...
            <li>
              <div class="collapsible-header grey lighten-3 grey-text">
                <span
                  >Owners <b class="light-blue-text">({{ authors.length }})</b></span
                >
              </div>
              <div class="collapsible-body noPadding">
                <div class="collection">
          ...
          <!-- FILTERS -->
          <div
            v-if="portal_name == 'translator'"
            class="p-1 grey lighten-4 rounded grey-text"
          ...
          >
            ...
            <template
              v-for="filter in all_filters['info.x-translator.component']"
              :key="filter.name"
            >
              <span
                @click.prevent="
              ...
              >
                <i class="material-icons tiny" :style="{ color: filter.color }">brightness_1</i
                >&nbsp;{{ filter.name }}&nbsp;<b class="light-blue-text" v-if="filter.count"
                  >({{ filter.count }})</b
                >
              </span>
            </template>
          ...
          <ul
            ...
            v-show="popularTags && popularTags.length > 5"
          >
                ...
                <div class="collection">
                  <div class="colllection-item padding20">
                    <template v-for="author in authors" :key="author">
                      <a
                        :class="{ active: author.active, blue: author.active }"
                        href="javascript:void(0)"
                        class="chip"
                        @click.prevent="
            ...
            <form class="col s12 center-align" @submit.prevent="search()" id="api_search_form">
              <template v-if="specialTagsUI">
                <div class="col s12 l3 input-field center-align">
                  <a
                    class="tooltipped p-1"
                    data-position="bottom"
                    :data-tooltip="'Learn More about ' + specialTagName"
                    :href="specialTagURL"
                    target="_blank"
                ...
                <div class="col s12 l6">
                  <input
                    ...
                    name="query"
                    autocomplete="off"
                    placeholder="Enter any query term here"
                    class="browser-default grey lighten-5 blue-grey-text lighter"
                  />
                </div>
              </template>
              <template v-else>
              ...
              <div class="col s12 selected-tags">
                <template v-for="tag in tags" :key="tag">
                  <div
                    ...
                    v-if="tag.active"
                    :class="{
                      purple: tag.name.toLowerCase() === specialTagOriginalName.toLowerCase(),
                      'white-text': tag.name.toLowerCase() === specialTagOriginalName.toLowerCase()
                    }"
                  >
                    {{ tag.name }}
                    <i
              ...
              <div class="col s12">
                <button
                  type="submit"
                  @click="googleAnalytics('Registry_Searches', query)"
                      ...
                      <div class="col s5 m10">
                        <template v-if="total && total > 0">
                          ...
                          <CopyButton
                            ...
                            @click.prevent="
                              googleAnalytics('Registry_SharedURL', window.location.href)
                            "
                            copy_msg="URL copied"
                            :copy="shareURL"
                          >
