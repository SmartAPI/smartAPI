    ...
    <div class="col s12 left">
      <small class="d-block">
        <a target="_blank" rel="noreferrer" :href="'/registry?q=' + item.association.smartapi.id">
          ...
          <i
            @mouseenter="highlightRowAndZoom(item)"
            @mouseleave="recenterGraph()"
            class="fa fa-search-plus pointer blue-grey-text"
            aria-hidden="true"
          ></i>
        </a>
        <i
          class="fa fa-info-circle pointer green-text"
          :class="'resultInfo' + badgeID"
          aria-hidden="true"
          style="float: right"
        ></i>
      </small>
      ...
      tippy('.resultInfo' + self.badgeID, {
        ...
        onShow(instance) {
          if (self.hoverInfo) {
            ...
            axios
              .get(
                '/api/query?size=1&q=_id:' +
                  info.association.smartapi.id +
                  '&fields=info.description,_meta.uptime_status'
              )
              .then((res) => {
                ...
                if (res.data.hits.length) {
                  ...
                  instance.setContent(
                    `<div>
                    <h6>API: <a target="_blank" href="/registry?q=` +
                      info.association.smartapi.id +
                      `">` +
                      info.association.api_name +
                      `</a></h6>
                    <p><small>` +
                      desc +
                      `<a target="_blank" href="/registry?q=` +
                      info.association.smartapi.id +
                    ...
                    <tbody>
                        ...
                        <td>
                            <small>` +
                      info.association.subject +
                      `/` +
                      info.association.input_id +
                      `</small>
                        </td>
                        </tr>
                      ...
                      `</small>
                        ...
                        <td>
                            <small>` +
                      info.association.object +
                      `/` +
                      info.association.output_id +
                      `</small>
                        </td>
                        </tr>
                    </tbody>
